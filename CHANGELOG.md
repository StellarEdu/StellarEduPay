...
- **`price_feed_stale` alert was unreachable — dead `_recordStale(...)` calls removed**: `currencyConversionService.getRates`' both-providers-failed path called a nonexistent `_recordStale(...)` helper three times (the real function is `_recordStaleness(provider, lastSuccessfulFetchMs)`). The resulting `ReferenceError` was swallowed by `getRates`' blanket `catch { return null; }`, so the `price_feed_stale` gauge was never set and the critical `PriceFeedStale` alert (`price_feed_stale == 1`, runbook `docs/runbooks/price-feed-staleness.md`) could never fire regardless of outage length. Replaced with an explicit `_markFeedStale(provider, stale)` helper that sets the gauge to 1 for all providers once both have failed past the stale window (or no cached rate exists) — the precondition of that code path is that the whole feed is degraded. The flag is self-healing: any successful fetch clears it back to 0 feed-wide (one working provider means conversions work again), while per-provider `price_feed_staleness_seconds` continues to show which provider lags. In the cached-but-exhausted branch, `_recordStaleness(cached.provider, cached.lastSuccessfulFetch)` now keeps the staleness age accurate. `getRates`' outer catch now logs the failure at error level before returning null, so programming errors on this path are visible instead of masquerading as cache misses. `docs/runbooks/price-feed-staleness.md` gained a "Signal Hierarchy" section documenting that `price_feed_stale == 1` is authoritative for prolonged outages and why the staleness-age and last-success-timestamp signals exist. Covered by `tests/priceFeedStale.test.js` (7 tests: flag raised in both exhausted branches, within-threshold serving keeps the flag down, recovery clears it, error logging asserted).
